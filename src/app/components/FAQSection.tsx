...
export function FAQSection() {
  const faqs = [
    ...
    },
    {
      ...
      answer:
        'Yes! Under the FAME II scheme, buyers can avail up to ₹1.5 lakh in central government subsidy. Additionally, many state governments offer additional incentives ranging from ₹50,000 to ₹1.5 lakh. Road tax exemptions and registration fee waivers are also available in most states.',
    },
    {
    ...
    },
    {
      ...
      answer:
        'Electric vehicles have significantly lower maintenance costs compared to traditional vehicles. EVs have fewer moving parts, no oil changes, and reduced brake wear due to regenerative braking. Annual maintenance typically costs around ₹8,000-12,000, which is 60% less than conventional vehicles.',
    },
    {
    ...
    <section id="faq" className="relative py-24 overflow-hidden">
      ...
      <div className="relative z-10 max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <motion.div
          ...
          whileInView={{ opacity: 1, y: 0 }}
          viewport={{ once: true }}
          className="p-8 rounded-3xl bg-white dark:bg-[#1A1F2E] backdrop-blur-sm border border-[#00C853]/10 dark:border-[#00C853]/20 shadow-sm"
        >
          <Accordion type="single" collapsible className="space-y-4">
            {faqs.map((faq, index) => (
              <AccordionItem
                key={index}
                value={`item-${index}`}
                className="border border-[#00C853]/10 dark:border-[#00C853]/20 rounded-2xl overflow-hidden bg-white dark:bg-[#2A2F3E] backdrop-blur-sm hover:border-[#00C853]/50 transition-all duration-300 shadow-sm"
              >
                <AccordionTrigger className="px-6 py-4 text-left font-['Poppins'] font-semibold text-[#0A0F1C] dark:text-white hover:text-[#00C853] transition-colors duration-300 [&[data-state=open]]:text-[#00C853]">
                  {faq.question}
                </AccordionTrigger>
                <AccordionContent className="px-6 pb-4 font-['Inter'] text-[#0A0F1C]/70 dark:text-white/70 leading-relaxed">
                  {faq.answer}
                </AccordionContent>
              </AccordionItem>
            ))}
